...
# Рядок 12 (index 11) — назви інгредієнтів, cols 4–79 (76 інгредієнтів)
...
for row in raw[12:]:  # дані з R13 (index 12)
    col0 = str(row[0]).strip() if row[0] else ''
    col1 = str(row[1]).strip() if row[1] else ''
    col2 = str(row[2]).strip() if row[2] else ''

    # Нова назва дня (col0 = "Понеділок, 06.04.2026" тощо)
    if col0:
        for d in DAYS_ORDER:
    ...
    # Прийом їжі (col1 = "Сніданок" / "Обід" / "Вечеря")
    ...
    if col1 == 'Усього за день':
        ing_vals = [fmt(row[i]) if i < len(row) else '' for i in range(4, 80)]
        days[cur_day].append({
            'meal': '__total__', 'dish': 'Усього за день',
            'pct': '', 'total': fmt(row[80]), 'meat': fmt(row[81]),
            'ings': ing_vals,
        })
        continue
